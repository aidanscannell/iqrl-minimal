   ...
   \FOR{$i$ {\bfseries to} $N_{\text{episodes}}$}
    ...
    \FOR{$i=1$ {\bfseries to} $T$}
        ...
        \IF{$i$ \% 2 == 0}
          ...
          \STATE $\quad \quad \eta \leftarrow \eta + \alpha \nabla \left( \mathcal{L}_{\pi}(\eta; \mathcal{D}) \right)$  \COMMENT{\cref{eq:policy-loss}}
          % \STATE $\bar{\eta} \leftarrow \tau \bar{\eta} + (1-\tau) \eta$
        \ENDIF
        \STATE Update target networks:
        \STATE $\quad \quad [\bar{\theta}, \bar{\psi}, \bar{\eta}] \leftarrow \tau [\bar{\theta}, \bar{\psi}, \bar{\eta}] + (1-\tau) [{\theta}, {\psi}, \eta]$
    \ENDFOR
   \ENDFOR
\end{algorithmic}
\end{algorithm}
...
\end{align}
where we use policy smoothing by adding clipped Gaussian noise $\epsilon_{t+n} \sim \text{clip} \left(\mathcal{N} (0,\sigma^{2}), -c, c \right)$ to the
...
\begin{align} \label{eq:policy-loss}
 \mathcal{L}_{\pi} = - \E_{o_{t} \sim \mathcal{D}} \left[ \min_{k\in\{1,2\}} Q_{\psi_{k}}(e_{\bar{\theta}}(o_{t}), \pi_{\eta}(e_{\bar{\theta}}(o_{t}))) \right].
\end{align}
%

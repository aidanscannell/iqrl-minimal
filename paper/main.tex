...
\begin{abstract}
  % Arno's proposal
  In reinforcement learning (RL) for continuous control, we propose  an efficient representation learning method using a latent-state consistency loss. This approach employs an encoder and a transition model to map observations to latent states and predict future states, respectively. By minimizing discrepancies between these states, we achieve task-agnostic representations and prevent representation collapse using only the consistency loss. Our method is straightforward, compatible with any model-free RL algorithm, and demonstrates state-of-the-art performance in continuous control benchmarks from DeepMind Control Suite.\looseness-1
...
\end{tikzpicture}
\caption{\textbf{\our's model architecture} \our learns a representation soley with the latent-state consistency loss, \ie a self-predictive loss, see \cref{eq:rep-loss}. We follow Finite Scalar Quantization (FSQ) \cite{mentzerFiniteScalarQuantization2023} and bound the domain of our latent representation, \ie we normalize it. To this end, our normalization results in an implicit codebook whose size we can control. We denote our normalization scheme by \cube.}
\label{fig:diagram-self-predictive}
\vskip -0.2in
\end{figure}


To the best of our knowledge, all prior works learn representations with objectives that depend on
...
As such, these methods learn task-specific representations.
...
It is based soley on a self-predictive loss.
...
Importantly, our representation {\em (i)} significantly speeds up learning, {\em (ii)} is simple to implement,

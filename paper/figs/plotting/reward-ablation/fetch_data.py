...
def fetch_results(run_path, run_name_list, keys, agent_name=None):
    ...
    for run_name in run_name_list:
        ...
        if not wandb_run.config["agent"]["reward_loss"]:
            ...
            wandb_run.config["agent"]["use_cosine_similarity_dynamics"]
            and wandb_run.config["agent"]["use_fsq"]
        ):
            history["name"] = "iQRL+cos+rew"
        elif (not wandb_run.config["agent"]["use_fsq"]) and wandb_run.config["agent"][
            "latent_dim"
        ] > 50:
